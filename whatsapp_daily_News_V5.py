...
TEST_MODE = False  # Set True for immediate test, False for scheduled only
...
if __name__ == "__main__":
    ...
    # TEST MODE - Optional immediate test
    if TEST_MODE:
        print("\n🧪 LIVE TEST - Real headlines NOW...")
        send_morning_news()
        print("\n🔄 DAILY SCHEDULE STARTING...")
    
    print("\n🔄 DAILY SCHEDULE:")
    schedule.every().day.at(f"{NEWS_HOUR:02d}:{NEWS_MINUTE:02d}").do(send_morning_news)
    print(f"• {NEWS_HOUR:02d}:{NEWS_MINUTE:02d} daily → Fresh news")
